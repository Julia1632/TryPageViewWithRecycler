...
import com.trypageviewwithrecycler.cache.BaseDiskCache;
import com.trypageviewwithrecycler.cache.DiskCache;
import com.trypageviewwithrecycler.streams.FileStreamProvider;
import com.trypageviewwithrecycler.streams.HttpStreamProvider;
import com.trypageviewwithrecycler.util.IOUtils;
    ...
    private class ImageResultAsyncTask extends AsyncTask<Void, Void, ImageResult> {
        ...
        protected ImageResult doInBackground(Void... voids) {
            ...
            try {
                ...
                Bitmap bitmap;
                Log.e(TAG, "doInBackground: " + config.cacheDir);
                if (config.hasDiskCache()) {
                    try {
                        ...
                        Log.e(TAG, "doInBackground: can't get file for " + request.url);
                    }
                }


                InputStream inputStream = new HttpStreamProvider().get(request.url);
                Log.e(TAG, "doInBackground: jj" + inputStream.toString().length());

                bitmap = getScaledBitmap(inputStream, request.height, request.width);

                Log.e(TAG, "doInBackground: " + inputStream.toString().length());
                Log.e(TAG, "doInBackground: h " + request.height + "w " + request.width);
